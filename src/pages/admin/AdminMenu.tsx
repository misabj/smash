...
const defaultForm = {
    ...
    price: "",
    image: "",
    category: "burger" as MenuItemAPI["category"],
    tags: "",
    popular: false,
    active: true,
};
...
export default function AdminMenu() {
    ...
    const fetchItems = async () => {
        try {
            const data = await menuApi.getAllAdmin();
            setItems(data);
        } catch (err) {
            console.error(err);
        } finally {
    ...
    const toggleActive = async (item: MenuItemAPI) => {
        try {
    ...
    return (
        <div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold">Upravljanje menijem</h1>
                    <p className="text-white/40 text-sm mt-1">
                        {items.length} stavki ukupno
                    </p>
                </div>
                <button
                    onClick={openCreate}
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="min-w-0">
                                                <div className="font-medium text-sm truncate max-w-[200px]">
                                                    {item.name}
                                                    {item.popular && (
                                                        <span className="ml-2 text-xs bg-[var(--color-primary)]/20 text-[var(--color-primary)] px-1.5 py-0.5 rounded-full">
                                                            Popular
                                                        </span>
                                                    )}
                                                </div>
                        ...
                        >
                            ...
                            <div className="flex items-center justify-between p-5 border-b border-white/5 flex-shrink-0">
                                <h2 className="text-lg font-bold">
                                    {editingId ? "Izmeni stavku" : "Nova stavka"}
                                </h2>
                                <button
                                    onClick={() => setModalOpen(false)}
                            ...
                            <div className="p-5 space-y-4 overflow-y-auto flex-1">
                                    ...
                                    <div>
                                        ...
                                        <select
                                            ...
                                            onChange={(e) =>
                                                setForm({
                                                    ...form,
                                                    category: e.target.value as MenuItemAPI["category"],
                                                })
                                            }
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        placeholder="beef, bestseller, spicy"
                                        className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder:text-white/30 focus:outline-none focus:border-[var(--color-primary)] transition-colors"
                                    />
                                </div>
                                ...
                                <div className="flex items-center gap-6">
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        ...
                                        />
                                        <span className="text-sm text-white/70">Popularan</span>
                                    </label>
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input
                                            type="checkbox"
                            ...
                            <div className="p-5 border-t border-white/5 flex justify-end gap-3 flex-shrink-0">
                                ...
                                >
                                    ...
                                    {editingId ? "Sačuvaj izmene" : "Dodaj stavku"}
                                </button>
                            </div>
                        </motion.div>
                    </>
